                    ...
                    <div class="bg-canvas-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow group flex flex-col">
                        ...
                        @else
                            <div class="w-full h-48 bg-mint-green-glow flex items-center justify-center overflow-hidden">
                                @if($loop->index == 0)
                                    <img src="{{ asset('images/pengantar.jpg') }}" alt="{{ $course->title }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                @elseif($loop->index == 1)
                                    <img src="{{ asset('images/pinjol.jpg') }}" alt="{{ $course->title }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                @elseif($loop->index == 2)
                                    <img src="{{ asset('images/judol.jpg') }}" alt="{{ $course->title }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                @else
                                    <img src="{{ asset('images/investasi.jpg') }}" alt="{{ $course->title }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                @endif
                            </div>
                        @endif
